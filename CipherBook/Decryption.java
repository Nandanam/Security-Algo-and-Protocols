...
/**
 * This class is responsible for Decrypting a ciphertext into original plain text.
 * Implements methods of AES Decryption to reverse the Encryption on plaintext
 *
...
public class Decryption {
    ...
    // Inverse S-Box array for reversing the SBOX substitution
    ...
    private static String[][] allKeysMatrix = new String[4][44];

    static int wcol = 44;
    static int round = 11;
    public static int colVal = 4;
    static int loop;

    /**
     * This method is called from Activity2.java to decrypt ciphertext with a user sent key
     *
     * @param k    Key sent by the user on UI
     * @param text ciphertext on UI
     * @return Returns a Hexadecimal String as plaintext
     */
    public static String aes(String k, String text) {
        String key = toHex(k);
        //String text = unHex(t);
        String plain = "";
        ...
        colVal = inputLength;
        int count1 = 0;
        //Generating Ciphertext matrix
        text = text.toUpperCase();
        for (int column = 0; column < 4; column++) {
            for (int row = 0; row < 4; row++) {
                userText[row][column] = text.substring(count1, count1 + 2);
                count1 = count1 + 2;
            }
        }
        try {
            int count = 0;
            for (int column = 0; column < colVal; column++) {
                for (int row = 0; row < 4; row++) {
                    userKey[row][column] = key.substring(count, count + 2);
                    count = count + 2;
                }
            }
            try {
                ...
                for (int row = 0; row < 4; row = row + 1) {
                    for (int column = 0; column < colVal; column++) {
                        allKeysMatrix[row][column] = userKey[row][column];
                    }
                }
                for (int column = colVal; column < wcol; column++) {

                    if (column % colVal != 0) {
                        if (column % 4 == 0 && colVal == 8) {
                            for (int row = 0; row < 4; row++) {
                                allKeysMatrix[row][column] = aesSBox(allKeysMatrix[row][column - 1]);
                                allKeysMatrix[row][column] = XOR(
                                        allKeysMatrix[row][column], allKeysMatrix[row][column
                                                - colVal]);
                            }
                        } else {
                            for (int row = 0; row < 4; row++) {
                                ...
                                allKeysMatrix[row][column] = XOR(
                                        allKeysMatrix[row][column - colVal],
                        ...
                        wNew[0][0] = allKeysMatrix[1][column - 1];
                        wNew[0][1] = allKeysMatrix[2][column - 1];
                        wNew[0][2] = allKeysMatrix[3][column - 1];
                        wNew[0][3] = allKeysMatrix[0][column - 1];
                        for (int i = 0; i < 4; i++) {
                            wNew[0][i] = aesSBox(wNew[0][i]);
                        }
                        String rconVal = aesRcon(column);
                        wNew[0][0] = XOR(wNew[0][0], rconVal);
                        for (int row = 0; row < 4; row++) {
                            allKeysMatrix[row][column] = XOR(allKeysMatrix[row][column
                                    - colVal], wNew[0][row]);
                        }
        ...
        int count = 0;
        for (int cval = allKeysMatrix[0].length - 4; cval >= 0 && count <= round - 1; cval -= 8) {

            for (int col = 0; col < 4 && cval < allKeysMatrix[0].length; col++, cval++) {
                for (int row = 0; row < 4; row++) {
                    keyMatrix[row][col] = allKeysMatrix[row][cval];
                }
            }

            if (count == 0 || count == round - 1) {
                userText = aesStateXor(userText, keyMatrix);
                if (count == 0) {
                ...
                userText = roundCalculation(userText, keyMatrix);
                loop = count++;
            }
        }
        for (int cols = 0; cols < 4; cols++) {
            for (int row = 0; row < 4; row++) {
                plain = plain + userText[row][cols];
            }
        }
        plain = unHex(plain);
        return plain;
     ...
     *
     * @param inHex String which is split and used as index on s_box
    ...
    /**
     * This method performs inverse SBOX substitution
     *
     * @param inHex Transformed plaintext
     * @return returns a string after inverse substitution
     */
    private static String inverseXBox(String inHex) {
        int rowNumber = Integer.parseInt(inHex.substring(0, 1), 16);
        int columnNumber = Integer.parseInt(inHex.substring(1, 2), 16);
        return INV_S_BOX[rowNumber][columnNumber];
    }

    /**
     * This method takes a String and returns an element from R_CON table
     *
     * @param round Index to lookup in R_CON matrix
     * @return Value from the R_CON matrix
     */
    private static String aesRcon(int round) {
        int rowNumber = 0;
        return R_CON[rowNumber][round / colVal];
    }
    ...
    /**
     * calls various functions for decryption in rounds
     *
     * @param text transformed Ciphertext
     * @param key  round key
     * @return
     */
     ...
     *
     * @param cHex   ciphertext matrix(transformed)
    ...
    public static String[][] aesStateXor(String[][] cHex, String[][] keyHex) {
        ...
        for (int row = 0; row < 4; row++) {
            for (int column = 0; column < 4; column++) {
                outputXor[row][column] = XOR(cHex[row][column],
                        keyHex[row][column]);
            }
        }
     ...
     *
     * @param inStateHex ciphertext matrix(transformed)
    ...
    /**
     * Substitutes from inverse-Sbox table
     *
     * @param inStateHex Transformed ciphertext for Nibble substitution from the inverseSBOX
     * @return return return String matrix
     */
    public static String[][] aesNibbleSub(String[][] inStateHex) {

        for (int row = 0; row < 4; row++) {
            for (int column = 0; column < 4; column++) {
                inStateHex[row][column] = inverseXBox(inStateHex[row][column]);
            }
        }
        return inStateHex;
    }
    ...
    public static String[][] aesMixColumn(String[][] Key) {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                System.out.print(aesMixColumnMatrix[i][j].toUpperCase());
            }
            System.out.println(" ");
        }
